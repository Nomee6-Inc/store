...
$query31 = mysqli_query($conn,"SELECT * FROM orders WHERE orderid = '$getorderid'");
$result31 = $query31->fetch_assoc();   
$getordersellertoken = $result31['seller'];

if($getsellertoken != $getordersellertoken) {
...
$createrandmd5hash = md5(rand().rand());
$target_file2 = $target_dir . $createrandmd5hash.".$imageFileType";
...
if ($uploadOk == 0) {
  ...
  if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file2)) {
    $getdeliverynote = $_POST['message'];
    $sql10 = "UPDATE orders SET 
        deliveryimg = '$createrandmd5hash.$imageFileType',
        deliverynote = '$getdeliverynote',
        status = 'Teslimat Tamamlandı'
        ...
        <div class="page-body">
          <div class="container-xl">
                <form action="" method="POST" enctype="multipart/form-data">
                  <div class="mb-3">
                        <label class="form-label">Teslimat Mesajı</label>
                  ...
                  <input type="submit" name="sendproduct" class="btn btn-primary" value="Teslim Et">
                </form>
            </div>
        </div>
